...
//        $dados = $query->groupBy('comissoes_corretores_lancadas.comissoes_id')->get();
//
//        dd($dados);



        $users = User::where("name" ,"!=",'Administrador')
            ->whereRaw("name != ''")
            ->get();
    ...
    public function geralIndividualPendentes(Request $request)
    {
        ...
        $cacheKey = 'geralIndividualPendentes_' . '_' . ($mes ?? 'todos_meses');
...
//                                $dataOriginal = $cells[17]->getValue();//21/01/2025
//
//                                $novoDia = $dia; // Coluna 18 Aqui o resultado e 30
...
//                                $data_vigencia = $dataDateTime->format('Y-m-d');
                                ...
                                $dataOriginal = $cells[17]->getValue(); // 21/01/2025
                                $novoDia = $dia; // Coluna 18 Aqui o resultado é 30
                                $dataDateTime = \DateTime::createFromFormat('d/m/Y', $dataOriginal);
                                //$dataDateTime->modify('+1 month');
    ...
    {
        $users = User::where("id","!=",1)->where('ativo',1)->get();
        $origem_tabela = TabelaOrigens::all();
...
//
//        $filename = uniqid() . '.xlsx'; // Gera um nome único
//        $destinationPath = storage_path("app/temp"); // Caminho para salvar
//
//
//        if (!$arquivo->move($destinationPath, $filename)) {
//            return response()->json(['error' => 'Erro ao mover arquivo!'], 400);
//        }
//
...
//        foreach ($reader->getSheetIterator() as $sheet) {
//            foreach ($sheet->getRowIterator() as $rowNumber => $row) {
//                if ($rowNumber <= 1) continue; // Pula a primeira linha
//
//                $cells = $row->getCells();
//                $nome = $cells[7]->getValue();
//                $codi = $cells[4]->getValue();
//                $carteirinha = $cells[5]->getValue();
//                $created_at = optional($cells[6]->getValue())->format('Y-m-d') ?? null;
//                $status = $cells[11]->getValue();
//                $valor_pago = $cells[10]->getValue();
//                $dt_pagamento = optional($cells[12]->getValue())->format('Y-m-d') ?? null;
//                $vencimento = optional($cells[13]->getValue())->format('Y-m-d') ?? null;
//
//                // Verifica se o cliente já existe pelo nome e código
//                if ($id_alt = $this->verificarCarteirinha($nome, $codi)) {
//                    $cliente = Cliente::find($id_alt);
//                    if ($cliente) {
...
//                        if ($contrato) {
//                            $contrato->created_at = $created_at;
//                            $contrato->save();
//                        }
//                    }
//
//                    // Se estiver liquidado, processa as comissões
//                    if (in_array($status, ["LIQUIDADO", "LIQUIDADO N/COB"])) {
//                        $this->atualizarComissoes($cliente, $vencimento, $dt_pagamento, $valor_pago);
//                    }
//                } else {
//                    // Verifica se a carteirinha já existe
//                    $cliente = Cliente::whereHas('contrato', function ($query) use ($carteirinha) {
//                        $query->whereRaw("LEFT(cateirinha, 11) = ?", [$carteirinha]);
//                    })->with('contrato.comissao.comissoesLancadas')->first();
//
//                    if ($cliente && in_array($status, ["LIQUIDADO", "LIQUIDADO N/COB"])) {
//                        $this->atualizarComissoes($cliente, $vencimento, $dt_pagamento, $valor_pago);
//                    }
//                }
//            }
...
//
//        $reader->close();
//        Storage::delete($filePath); // Remove o arquivo temporário após o processamento
//
//        return "successo";
//    }
//
//    /**
//     * Atualiza as comissões com base nos dados do arquivo
//     */
//    private function atualizarComissoes($cliente, $vencimento, $dt_pagamento, $valor_pago)
//    {
//        if (!$cliente) return;
//
//        $comissoes = $cliente->contrato->comissao->comissoesLancadas ?? collect();
//        $datasCadastradas = ComissoesCorretoresLancadas::whereIn('comissoes_id', $comissoes->pluck('comissoes_id'))
//            ->whereNotNull('data')
//            ->pluck('data');
//
//
...
//
//        if ($dataMenor) {
//            ComissoesCorretoresLancadas::whereIn('comissoes_id', $comissoes->pluck('comissoes_id'))
//                ->where('parcela', 1)
//                ->update([
//                    'data' => $dt_pagamento,
//                    'data_baixa' => $dt_pagamento
...
//                ]);
//
//                // Atualiza o status financeiro do contrato de acordo com a parcela
//                $statusFinanceiro = match ($c->parcela) {
//                    2 => 6,
//                    3 => 7,
//                    4 => 8,
//                    5 => 9,
...
//
//                if ($statusFinanceiro) {
//                    Contrato::where('id', $c->comissoes->contrato_id)->update(['financeiro_id' => $statusFinanceiro]);
//                }
//            }
//        }
//    }
